...
import { OUTLETS, RESTAURANT_CITY_FILTERS, RESTAURANT_VIBE_FILTERS } from "../../../data/home/restaurant/RestaurantsSection.data";
    ...
    <section className="sticky top-20 z-40 bg-white shadow-md">
      <div className="container-custom py-2">
        <button type="button" onClick={() => setIsOpen((open) => !open)} aria-expanded={isOpen} className="flex w-full items-center justify-between py-2 text-sm font-semibold text-gray-700 transition-colors hover:text-primary-600">
          <span className="flex items-center gap-2"><ChevronDown className={`size-4 transition-transform ${isOpen ? "rotate-180" : ""}`} />Filters</span><span className="text-xs text-gray-600">{OUTLETS.length} outlets</span>
        </button>
        <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="space-y-3 pt-3 pb-2">
            ...
            <FilterGroup label="Filter by Vibe" filters={RESTAURANT_VIBE_FILTERS} activeFilter={vibe} onChange={onVibeChange} activeClass="bg-primary-600 text-white shadow-md" />
          </div>
        </div>
      </div>
    </section>
...
type FilterGroupProps = { label: string; filters: readonly string[]; activeFilter: string; onChange: (filter: string) => void; activeClass: string };
