...
import { History, Loader2, Phone, RotateCcw, ShieldCheck, X } from "lucide-react";
      ...
      <div onClick={(event) => event.stopPropagation()} className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-3xl border border-white/10 bg-[#0b0b0b] p-6 shadow-2xl md:p-8">
        ...
        {step === "phone" && (
          <form className="space-y-4" onSubmit={handleSendOtp}>
            ...
            <button type="submit" disabled={sendingOtp || phone.length !== 10} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary-600 py-3.5 text-sm font-semibold tracking-wide text-white transition-all duration-300 hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        {step === "otp" && (
          <form className="space-y-4" onSubmit={handleVerifyOtp}>
            <p className="text-sm text-white/50">Enter the 6-digit code sent to <span className="text-white">{phone}</span>.</p>
            <div>
            ...
            <button type="button" onClick={() => setStep("phone")} className="flex w-full items-center justify-center gap-1.5 text-xs text-white/40 transition-colors hover:text-white/70">
              <RotateCcw className="size-3.5" />Use a different number
            </button>
          </form>
        )}
